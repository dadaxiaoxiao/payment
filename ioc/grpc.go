...
func InitGRPCServer(ecli *clientv3.Client,
	wechatsvc *grpc2.WechatServiceServer, redisClient redis.Cmdable, l accesslog.Logger) *grpcx.Server {
	type Config struct {
		Port    int   `yaml:"port"`
	...
	// 限流器
	limiter := pkgratelimit.NewRedisSlideWindowLimiter(redisClient, pkgratelimit.WithInterval(time.Second), pkgratelimit.WithRate(1000))

	// 这里添加拦截器
	server := grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			// prometheus 拦截器
			prometheus.NewInterceptorBuilder("qinye", "webook_payment").BuildServer(),
			// 链路追踪
